...

{
    ...
    public Fox mateTarget;

    public override void Init(Coord coord, Environment env)
    {
        base.Init(coord,env);
        this.moveSpeed = env.moveSpeedFox;
        this.hungerResistance = env.hungerResistanceFox;
        this.thirstResistance = env.thirstResistanceFox;
        ...
        this.thirst = 0;
        this.reproduction = 0;
        foodQuantity = 1 / (this.hungerResistance*moveSpeed);
        drinkQuantity = 1 / (this.thirstResistance*moveSpeed);
        this.alreadyReproduced = false;
        currentAction = CreatureAction.None;

        material.color = (genes.isMale) ? maleColour : femaleColour;
        this.gameObject.name = (genes.isMale) ? "Fox(Male)" : "Fox(Female)";
    ...
    protected virtual void FixedUpdate()
    {
        ...
        hunger += Time.deltaTime * 1 / (this.hungerResistance * moveSpeed);
        thirst += Time.deltaTime * 1 / (this.thirstResistance * moveSpeed);
        if (reproduction > 1f)
        {
    ...
    protected virtual void ChooseNextAction()
    {
        ...
        else if (genes.isMale == false && 0.7 < reproduction && reproduction < 0.8 && alreadyReproduced == false)
        {
            FindMate();
        }
